...
class DataSplitter:
    ...
    def custom_train_test_split(self, features, labels, test_size=0.2, anomaly_ratio=0.01):
        ...
        print(f"Original data: {len(labels)} samples, {np.sum(labels == 1)} anomalies ({np.mean(labels) * 100:.2f}%)")
        print(
    ...
    def find_optimal_anomaly_ratio(self, features, labels, test_size=0.2, ratios=[0.01, 0.05, 0.1, 0.15, 0.2]):
        ...
        for ratio in ratios:
            ...
            print(f"  Anomaly precision: {report['1']['precision']:.4f}, recall: {report['1']['recall']:.4f}")
...
class GAOptimizer:
    ...
    def __init__(self, config, X, y):
        ...
        self.int_params = ['max_depth', 'min_child_weight']  # Add any other integer parameters here
    ...
    def optimize(self):
        ...
        print(f"Starting GA optimization with {self.population_size} individuals over {self.generations} generations")
