...
class DataProcessor:
    ...
    def preprocess_sequence(self, sequence):
        ...
        n_grams = []
        for n in range(self.config["n_gram_range"][0], self.config["n_gram_range"][1] + 1):
            n_grams.extend(
    ...
    def process_data(self, df):
        ...
        processed_cols = [f"processed_{col}" for col in self.config["input_columns"].values() if
                          f"processed_{col}" in df.columns]
        if processed_cols:
        ...
        for column, rules in self.config["anomaly_rules"].items():
            ...
            if column in df.columns:
                anomaly_results = df[column].apply(lambda x: self.check_sequence_anomalies(x, rules))
                for i, rule in enumerate(rules):
                    col_name = f"{column}_anomaly_{i}"
                    df[col_name] = anomaly_results.apply(lambda x: x[i])
                    print(f"Created anomaly column: {col_name}")
            else:
                print(f"Warning: Column '{column}' not found in the dataset. Skipping its rules.")
...
class FeatureEngineer:
    ...
    def create_features(self, df, vectorizer=None, mlb=None):
        ...
        print(f"Columns in df: {df.columns if isinstance(df, pd.DataFrame) else 'Not a DataFrame'}")
        ...
        if 'combined_sequence' not in df.columns:
            print("Warning: 'combined_sequence' not found in DataFrame. Available columns:", df.columns)
        ...
        cat_columns = [f"{col}_encoded" for col in self.config["categorical_columns"]
        ...
        if multi_value_columns and len(multi_value_columns) > 0 and multi_value_columns[0] in df.columns:
            if mlb is None:
                mlb = MultiLabelBinarizer()
                multi_value_features = mlb.fit_transform(df[multi_value_columns].iloc[:, 0])
            else:
        ...
        if not anomaly_columns:
            print("No anomaly columns found. Check your anomaly rules in the configuration.")
    ...
    def analyze_feature_importance(self, model, feature_names, top_n=10):
        ...
        importance_df = pd.DataFrame({
            "Feature": [feature_names[int(k[1:])] if k[1:].isdigit() and int(k[1:]) < len(feature_names)
                        else k for k in importance_dict.keys()],
    ...
    def suggest_features(self, df, importance_df=None):
        ...
        if "DISPUTE_ID" in df.columns:
            suggestions.append("Consider adding temporal features like day of week or time of day")

        # Check for network-based features
        if any("SEQUENCE" in col for col in df.columns):
            suggestions.append("Consider adding network-based features like sequence length or complexity")

        # Suggest embeddings for sequence data
        if any(col in df.columns for col in self.config["input_columns"].values()):
            suggestions.append("Consider using advanced sequence embeddings like BERT or Word2Vec")
